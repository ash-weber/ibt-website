...
const SETTINGS_KEYS = {
  ...
  ABOUT_WHO_TITLE: "about_who_title",
  ABOUT_WHO_DESCRIPTION: "about_who_description",
  ABOUT_WHO_SECONDARY_DESCRIPTION: "about_who_secondary_description",
  ABOUT_WHO_FEATURES: "about_who_features",
  ABOUT_WHO_IMAGES: "about_who_images",
  ABOUT_PROCESS_BADGE: "about_process_badge",
  ABOUT_PROCESS_TITLE: "about_process_title",
  ABOUT_PROCESS_FEATURES: "about_process_features",
  ABOUT_PROCESS_IMAGE: "about_process_image",
  ABOUT_MISSION_TITLE: "about_mission_title",
  ABOUT_MISSION_DESC: "about_mission_desc",
  ABOUT_VISION_TITLE: "about_vision_title",
  ABOUT_VISION_DESC: "about_vision_desc",
  ABOUT_MISSION_CARDS: "about_mission_cards",
  ABOUT_CONTACT_BADGE: "about_contact_badge",
  ABOUT_CONTACT_TITLE: "about_contact_title",
  ABOUT_CONTACT_IMAGE: "about_contact_image",
};
...
async function main() {
  ...
  // Mimic saveSettings payload check
  console.log("Simulating saveSettings changes to detect validation errors:");
  for (const [formKey, defaultValue] of Object.entries(DEFAULT_VALUES)) {
    const dbKey = SETTINGS_KEYS[formKey as keyof typeof SETTINGS_KEYS];
    if (!dbKey) continue;

    const valueInForm = formValues[formKey];
    const valueInDb = currentMap.get(dbKey);

    // If it differs, or if it is not in db (so it will be upserted)
    if (valueInForm !== valueInDb) {
      const schema = settingSchemas[dbKey];
      if (!schema) continue;

      const parsed = schema.safeParse(valueInForm);
      if (!parsed.success) {
        console.log(`❌ FAILED: ${dbKey}`);
        console.log(`   Form Value: ${JSON.stringify(valueInForm)}`);
        console.log(`   Db Value:   ${JSON.stringify(valueInDb)}`);
