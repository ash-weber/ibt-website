...
import { getAllTestimonialsMasterItems, updateTestimonialsMasterItem } from '../../../../api/testimonialsMaster'
...
const REORDER_QUERY_KEY = ['master-testimonials-reorder']
...
function sortTestimonials(items: TestimonialMasterItem[]) {
  return [...items].sort((a, b) => a.order - b.order)
}

function SortableReorderCard({ testimonial }: { testimonial: TestimonialMasterItem }) {
...
export function TestimonialsReorderPage() {
  ...
  const { data, isPending, isError, error, refetch } = useQuery({
    ...
    queryFn: getAllTestimonialsMasterItems,
    staleTime: 0,
    gcTime: 0,
  ...
  const reorderMutation = useMutation({
    mutationFn: async (orderedIds: string[]) => {
      for (const [index, id] of orderedIds.entries()) {
        await updateTestimonialsMasterItem(id, {
          order: index + 1,
        })
      }
    ...
    onSuccess: () => {
      ...
      setToastMessage('Testimonial order updated.')
      setToastOpen(true)
    },
    onError: (err) => {
      refetch()
      setToastVariant('error')
      setToastMessage(getApiErrorMessage(err, 'Failed to reorder testimonials.'))
      setToastOpen(true)
    },
  })

  const testimonials = useMemo(() => sortTestimonials(data ?? []), [data])
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const reordered = arrayMove(testimonials, oldIndex, newIndex).map((item, index) => ({
      ...item,
      order: index + 1,
    }))

    queryClient.setQueryData<TestimonialMasterItem[]>(REORDER_QUERY_KEY, reordered)
      ...
      <div className="grid min-h-[calc(100vh-4.5rem)] place-items-center p-4">
        <div className="rounded-[var(--ui-radius-lg)] border border-[var(--ui-border)] bg-white px-6 py-5 shadow-[var(--ui-shadow-md)]">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-semibold text-[var(--ui-muted)]">Loading all testimonials for reorder...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="grid min-h-[calc(100vh-4.5rem)] place-items-center p-4">
        <div className="w-full max-w-xl rounded-[var(--ui-radius-lg)] border border-red-200 bg-red-50 p-8 text-center shadow-[var(--ui-shadow-md)]">
          ...
          <p className="mt-1 text-sm text-red-700">{getApiErrorMessage(error, 'Unable to load testimonials.')}</p>
  ...
  return (
    <div className="flex min-h-[calc(100vh-4.5rem)] w-full flex-col gap-4 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--ui-radius-lg)] border border-[var(--ui-border)] bg-white p-3 shadow-[var(--ui-shadow-sm)]">
        <ActionButton size="sm" intent="ghost" leftIcon={<FiChevronLeft />} onClick={() => navigate('/admin/master/testimonials')}>
          Back to Testimonials
        </ActionButton>

        <p className="flex items-center gap-2 text-xs text-[var(--ui-muted)]">
          <FiCheckCircle size={14} />
          Drag cards to reorder. All testimonials are loaded on this page.
        </p>
      </div>
