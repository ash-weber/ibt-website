...
import { getAllServicesMasterItems, updateServicesMasterItem } from '../../../../api/servicesMaster'
...
const REORDER_QUERY_KEY = ['master-services-reorder']
...
function sortServices(items: ServiceMasterItem[]) {
  return [...items].sort((a, b) => a.order - b.order)
}

function SortableReorderCard({ service }: { service: ServiceMasterItem }) {
...
export function ServicesReorderPage() {
  ...
  const { data, isPending, isError, error, refetch } = useQuery({
    ...
    queryFn: getAllServicesMasterItems,
    staleTime: 0,
    gcTime: 0,
  ...
  const reorderMutation = useMutation({
    mutationFn: async (orderedIds: string[]) => {
      for (const [index, id] of orderedIds.entries()) {
        await updateServicesMasterItem(id, {
          order: index + 1,
        })
      }
      return orderedIds
    ...
    onSuccess: () => {
      ...
      setToastMessage('Service order updated.')
      setToastOpen(true)
    },
    onError: (err) => {
      refetch()
      setToastVariant('error')
      setToastMessage(getApiErrorMessage(err, 'Failed to reorder services.'))
      setToastOpen(true)
    },
  })

  const services = useMemo(() => sortServices(data ?? []), [data])
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    queryClient.setQueryData<ServiceMasterItem[]>(REORDER_QUERY_KEY, reordered)
      ...
      <div className="grid min-h-[calc(100vh-4.5rem)] place-items-center p-4">
        <div className="rounded-[var(--ui-radius-lg)] border border-[var(--ui-border)] bg-white px-6 py-5 shadow-[var(--ui-shadow-md)]">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-semibold text-[var(--ui-muted)]">Loading all services for reorder...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="grid min-h-[calc(100vh-4.5rem)] place-items-center p-4">
        <div className="w-full max-w-xl rounded-[var(--ui-radius-lg)] border border-red-200 bg-red-50 p-8 text-center shadow-[var(--ui-shadow-md)]">
          ...
          <p className="mt-1 text-sm text-red-700">{getApiErrorMessage(error, 'Unable to load services.')}</p>
  ...
  return (
    <div className="flex min-h-[calc(100vh-4.5rem)] w-full flex-col gap-4 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--ui-radius-lg)] border border-[var(--ui-border)] bg-white p-3 shadow-[var(--ui-shadow-sm)]">
        <ActionButton size="sm" intent="ghost" leftIcon={<FiChevronLeft />} onClick={() => navigate('/admin/master/services')}>
          Back to Services
        </ActionButton>

        <p className="flex items-center gap-2 text-xs text-[var(--ui-muted)]">
          <FiCheckCircle size={14} />
          Drag cards to reorder. All services are loaded on this page.
        </p>
      </div>

      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        {services.length > 0 ? (
          <SortableContext items={services.map((item) => item.id)} strategy={verticalListSortingStrategy}>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {services.map((service) => (
                <SortableReorderCard key={service.id} service={service} />
              ))}
            </div>
          </SortableContext>
        ) : (
          <div className="rounded-lg border border-dashed border-[var(--ui-border)] p-6 text-center text-sm text-[var(--ui-muted)]">
            No services available to reorder.
          </div>
        )}
      </DndContext>
    </div>
