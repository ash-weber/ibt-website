...
import type { ActionIntent, ComponentSize } from '../types/ui'
...
export function ActionButton({
  ...
  const hasElementInChildren = Children.toArray(children).some((child) => isValidElement(child))
  const resolvedLeftIcon =
    leftIcon !== undefined
      ? leftIcon
      : hasElementInChildren
      ? undefined
      : DEFAULT_LEFT_ICON_MAP[intent]
  ...
  return (
    <button
      type="button"
      className={cx(BASE, SIZE_MAP[size], INTENT_MAP[intent], fullWidth && 'w-full', className)}
    ...
    >
      {loading ? (
        <>
          <Loader size="sm" label="Loading" className="border-white/40 border-t-white" />
          <span>{children}</span>
        </>
      ) : (
        <>
          {resolvedLeftIcon ? <span aria-hidden>{resolvedLeftIcon}</span> : null}
          <span>{children}</span>
